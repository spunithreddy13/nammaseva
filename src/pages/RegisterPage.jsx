        ...
        <div className="google-modal-overlay" onClick={() => setShowGoogleModal(false)}>
          <div className="google-modal" onClick={e => e.stopPropagation()}>
            <div className="google-modal__header">
              ...
              <button className="google-modal__close" onClick={() => setShowGoogleModal(false)}>✕</button>
            </div>
            <div className="google-modal__body">
              <div className="google-modal__spinner"><div className="google-modal__ring" /></div>
              <p className="google-modal__status">Connecting to Google...</p>
              <p className="google-modal__note">🔧 Google OAuth requires backend setup.<br/>This will be fully functional once the Node.js server is connected.</p>
            ...
            <button className="google-modal__dismiss btn btn--primary" onClick={() => setShowGoogleModal(false)}>Got it, I'll use email for now</button>
          </div>
        </div>
      )}

      {showAadhaarModal && (
        <div className="google-modal-overlay" onClick={() => setShowAadhaarModal(false)}>
          <div className="google-modal aadhaar-modal" onClick={e => e.stopPropagation()}>
            <div className="google-modal__header">
              ...
              <button className="google-modal__close" onClick={() => setShowAadhaarModal(false)}>✕</button>
            </div>
            <div className="google-modal__body">
              <p className="google-modal__note" style={{ marginTop: 0 }}>Aadhaar authentication uses <strong>UIDAI's OTP verification</strong> API and will be enabled once the backend is integrated.</p>
              <div className="aadhaar-modal__how">
                {['Enter your 12-digit Aadhaar number', 'Receive OTP on Aadhaar-linked mobile', 'Verify and register securely'].map((s, i) => (
                  <div key={i} className="aadhaar-modal__step">
                    <span className="aadhaar-modal__num">{i + 1}</span>
                    <span>{s}</span>
                  </div>
                ))}
              </div>
              <div className="aadhaar-modal__badge">🔒 &nbsp;100% secure · UIDAI certified · No data stored</div>
            </div>
            <button className="google-modal__dismiss btn btn--primary" onClick={() => setShowAadhaarModal(false)}>Understood, use email for now</button>
          </div>
        </div>
      )}
